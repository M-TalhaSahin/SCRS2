...
class Students:
    ...
    def toJson(self, fileName: str):
        ...
        with open(fileName, 'w', encoding='utf-8-sig') as jf:
            json.dump(jsonDict, jf, indent=4, ensure_ascii=False)
        ...
        for studentDict in jsonDict['students']:
            for semesterDict in studentDict['semesters']:
                for courseDict in semesterDict['courseGradeList']:
                    students.addSingleCourse(studentDict['studentId'], courseDict['courseName'], str((int(semesterDict['year']), int(semesterDict['period']))), courseDict['grade'], courseDict['credit'])
    ...
    def calculateGpaBeforePeriod(self, period: (int, int)) -> float:
        ...
        for key in self.semesterDict.keys():
            ...
            for i in range(self.semesterDict[key].course_grade_list.__len__()):
                kr = self.semesterDict[key].course_grade_list[i][2]
                grade = self.semesterDict[key].course_grade_list[i][1]
                sumOfP += kr * grade
                count += kr
        return (sumOfP/count) if count else -1


class Semester:
    def __init__(self, _period: (int, int)):
        self.period = _period
        self.course_grade_list = []  # (name: str, grade: float, credit: int)


if __name__ == "__main__":
    import shutil
    shutil.copyfile("data/trainStudents.json", "data/testing.json")
