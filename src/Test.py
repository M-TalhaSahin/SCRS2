...
class Test:
    def __init__(self, fileNames: dict):
        ...
        self.courseMatrix = pd.read_csv(fileNames['courseListCsvFileName'], index_col=0, na_values=['NA'], encoding='utf-8-sig').fillna(0).drop(columns=['KR', 'insanb']).replace(-1, 0)
        self.predList = []
        self.realList = []
        self.cutNumber = 3
        self.passGrade = 3.5
